...
> **コード進行作成支援アプリケーション**  
> 直感的なタイムライン、多彩な記法入力、柔軟なボイシング、そして DAW へのダイレクトな MIDI ドラッグ＆ドロップに対応した Windows 向けコード進行アシスタントツールです。
...
- **3 種類の内蔵シンセサイザー音色**:  
  ...
  - **Pad**: 和音の響きやボイシングの広がりを確認しやすい豊かな持続音
  - **Organ**: クリアな発音でコードの輪郭を捉えやすいオルガン音
- **音割れ防止ソフトリミッター**:  
  重低音や 8 音以上の密集テンション和音を最大音量で鳴らしても、クリッピングノイズ（音割れ）を起こさず安全に再生します。

### 3. 高度なボイシング設定・音域コントロール
- **転回形（Inversion）**:  
  基本形（Root）、第1転回形、第2転回形、第3転回形をワンクリックで切り替え可能。
- **オープンボイシング（Drop 2 等）**:  
  音の重なりを広げ、濁りのないリッチで広がりのあるサウンドへ自動ボイシング展開。
- **ベース音付加**:  
  コードの最低音に対して、1 オクターブ下（Root-12）、2 オクターブ下（Root-24）、あるいはその両方（Both）を自動追加し、楽曲の重心を支える低音を付加できます。
- **全体オクターブ・音域調整**:  
  タイムライン全体の音域（低音寄り・高音寄り）をスピーディーに変更可能。

### 4. 豊富な進行テンプレートとユーザー定義管理
- **50 種類以上の組み込みプリセット**:  
